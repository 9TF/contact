...
$event_date = date('Y-m-d',$event);
$query = "UPDATE `cb`.`contacts` SET `name` = '$name', `number` = '$number', `email` = '$email', `address` = '$address', `group` = '$group', `event_name` = '$event_name', `event_date` = '$event_date' WHERE `contacts`.`number` = $old";
require_once '../includes/db.inc.php';
...
<body >
        ...
        <div class="w3-col m7 l8" >
         ...
         <div  class=" w3-border w3-section w3-card-8 " style="min-height:250px;overflow: auto;" >
            <form class="w3-form" action="edit.php?number=<?php echo$row['number']; ?> " method="POST">
                <table class="w3-table">
                    <tbody>
                        ...
                        $groups = mysql_query($query);
                        while($group = mysql_fetch_assoc($groups)){ ?>
                        <option> <li class="fa fa-group"> <?php echo $group['group_name']; ?></li></option>
                        <?php } ?>
                        </select>
                        </td>
                        </tr>
                        <tr>
                        <td>Event Name</td>
                        <td><input  type="tetx" name="event_name" value="<?php echo $row['event_name']; ?>" /></td>
                        </tr>
                        <tr>
                        <td>Event Date</td>
                        <td><input class="datepicker" type="date" name="event_date" value="<?php echo $row['event_date']; ?>" /></td>
                        </tr>
                        <tr>
                            <td colspan="2"><input class="w3-btn" type="submit" name="update" value="update"/></td><td><input class="w3-btn"  type="button" value="cancel update" onclick="history.go(-1)"/></td>
                        </tr>
